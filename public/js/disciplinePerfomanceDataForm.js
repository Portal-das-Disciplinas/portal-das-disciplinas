...
function onClickSearchClasses(event,disciplineCode) {
    ...
    $.ajax({
        url: "/apisistemas/turmas?codigo-componente=" + codigoComponente + "&ano=" + ano + "&periodo=" + periodo,
        method: 'GET',

        success: function(result) {
            let html = "";
            result.forEach(function(turma) {
                html += "<option value='" + turma['id-turma'] + "'> Turma " + turma['codigo-turma'] + " - " + turma['descricao-horario'].split(" ")[0] + " </option>"
            });
...
function onSearchDisciplineDataClick(code) {
    if(!document.querySelector('#checkAllClasses').checked && document.querySelector('#selectClass').value=='' ){
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            if (result['soma-medias']) {
                ...
                let percentagemReprovados = ((result['quantidade-reprovados'] / result['quantidade-discentes']) * 100).toFixed(1);
